...
# grouping by ID, congruency (congruency), session (reactive/baseline), phase (test/retest)
# ACC, RT
mean_data <- raw_data |>
...
boot_icc <- function(data, indices, rt_weighting) {
  ...
  # pivot congruency back into long for scaling
  data_temp <- data_temp |>
    pivot_longer(
      cols = -c(ID, session),
      names_to = c(".value", "congruency"),
      names_pattern = "(.*)_(Congruent|Incongruent)"
    )
...
# check how many models didn't converge
mean(is.na(boot_ICC_obj$t))
...
# problems with convergence probably because of low variance in one component for some bootstrap samples
############ TESTING ##########
...
for (curr_mod in modalities){
  for (curr_weight in weights) {
    ...
    # extract bias corrected accelarated ci
    bca_ci <- boot.ci(boot_ICC_obj, type = "bca")
...
cuedts_plot <- ggplot(weight_table, aes(x = weight_rt, y = t0, group = modality, color = modality)) +
  geom_line() +
  geom_point(size = 2) +
  geom_errorbar(
...
ggsave(plot = cuedts_plot, file = 'study_1_reliability/plots/bootstrap_50_excl/cuedts_boot.png', width = 8, height = 4)
