...
boot_fun <- function(ids, indices) {
  ...
  for (curr_type in types_weight) {
    ...
    corr_data <- corr_matrix |>
      ...
      filter(var1 < var2) |> # exclude duplicates and auto correlations
      mutate(weight_type = curr_type) |> # save current weight
  ...
  corr_rt_opt <- corr_long |>
    filter(weight_type %in% c("rt", "optimal")) |>
    mutate(weight_type = relevel(factor(weight_type), ref = "optimal"))
  ...
  rt_fe_intercept <- unname(fixef(mlm_rt_opt)['(Intercept)'])
  rt_fe_weighttype <- unname(fixef(mlm_rt_opt)['weight_typert'])
  ...
  er_fe_intercept <- unname(fixef(mlm_er_opt)['(Intercept)'])
  er_fe_weighttype <- unname(fixef(mlm_er_opt)['weight_typeer'])
...
(boot_estimates |> filter(er_re_se_intercept == 0) |> nrow())/5000 * 100
...
boot.ci(boot_out, type = "perc", index = 2, conf = 0.9) # rt_fe_weighttype
boot.ci(boot_out, type = "perc", index = 5, conf = 0.9) # er_fe_weighttype


# inverse fisher z intercept
ci_data[1,3:5] # intercept rt
ci_data[4,3:5] # intercept er
